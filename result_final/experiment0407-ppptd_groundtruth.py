...
def TimeAllocate():
    ...
    global alpha, beta, w,scale,lamuda,scalewe,wwe,epsionoall,adaptive_users
    ...
    for ti in range(79):        #先考虑有20个时间戳
        ...
        #求每个时间戳下的ppptd的truth值和数据集原本的groundtruth差距
        truthlist,tdlist = ppptd_groundtruth(Zall,originals,ti) #返回的是每个时间戳下每个objects对应的真实值和估计真实值
        # final_truth = truthlist
        # final_td  = []
        # final_td = final_td.append(tdlist)
        # #把每个时间戳的ppptd和groundtruth存入txt
        # with open('./data/groundtruth/ground_'+str(ti)+'.txt', 'w') as f:
        #     f.write(str(truthlist))
        # with open('./data/groundtruth/td_'+str(ti)+'.txt', 'a') as f:
        #     f.write(str(tdlist))
    ...
    #
        rho = 0.02
        PI = 0.02
        directory = './/data//lamuda' + str(lamuda) + '_adaptive_user//' + str(maxepsiono) + '_' + str(PI) + '_' + str(
            rho) + '//'
        if not os.path.exists(directory):
            os.makedirs(directory)
        with open(directory + str(ti) + '.txt', 'a', encoding='utf-8') as f:
            f.write(str(adaptive_user) + '\n')
        with open(directory + str('all') + '.txt', 'a', encoding='utf-8') as f:
            f.write(str(adaptive_user) + '\n')
        adaptive_users = adaptive_users + adaptive_user
    print('总更新用户：', adaptive_users)
...
def ppptd_groundtruth(Zall,originals,ti):  # 传入的是这个时间戳的真值(一维列表）和原始值（二维列表，包含用户数和观测数），还有时间戳；
