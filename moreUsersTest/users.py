...
import math
import sympy
from file_test import Twodimension_Init
...
#epsiono=[]               #epsiono是用户端传上来的，也需要维护一个表吧？每一个用户对应着一个epsiono，同时也有时间标签，所以是一个和weight一样的二维数组

def perturbation(epsiono,originals,ti):        #三个参数：epsiono是对应用于加噪的参量，ti是当前时间戳
    xi=[]       #定义xi为扰动后的数据
    xi = Twodimension_Init(16,26,0.0001)          #xi的初始化——16个users、26个objects，初值为0.0001
    for user in range(len(originals)):                      #originals是在某一时间戳下用户的原始数据，是个二维数组，有多用户多object；
        for obj in range(len(originals[0])):                 #对同一个用户而言，它对应的观测量objects的加噪是   不一样的!!!
            xi[user][obj] = originals[user][obj] + np.random.laplace(0., epsiono[ti][user][obj], 1)
                                #加噪
