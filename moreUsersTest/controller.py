...
def TimeAllocate():
    ...
    for ti in range(39):        #先考虑有20个时间戳
        ...
        Zallwe,sumepsiono2 = Weight_Computation1(xiwe, ti, scalewe, wwe, Zallwe)
...
def initial(maxepsiono,A,B):
    ...
    users = 16     #预定义的用户数
    objects = 26     #预定义的任务数量
    epsiono1 = random.uniform(1,1.3)                    #高隐私级别
    epsiono2 = random.uniform((A/B)*maxepsiono-0.3,(A/B)*maxepsiono)     #这里的范围也需要进一步确定
