...
from estimation import MAE,MAPE,MAE2
from file_test import txt_read,Twodimension_Init,Threedimension_Init
import numpy as np
...
lamuda =0.5         #预定义的lamuda值，决定最开始的隐私分配
...
def TimeAllocate():
    ...
    epsionoall =initial(maxepsiono, A, B)           #这里不需要返回值，epsiono和weight都是全局变量
    ...
    Zallwe = []
    for ti in range(39):        #先考虑有20个时间戳
        originals = []      #原始数据，二维数组，user_obj

        originals = txt_read(ti)

        xi=perturbation(scale,originals,ti)
        scale,tilast=adaptive_sample(tilast,ti,scale,w)
        Zall = Weight_Computation(xi, ti, scale, w, Zall)  # w考虑返回

        first = MAE2(Zall, originals, ti)            #求这个时间戳下的mae
        Mae.append(first)
        second = MAPE(Zall, originals, ti)          #求这个时间戳下的mape
        Mape.append(second)

        xiwe = perturbation(scalewe, originals, ti)
        Zallwe = Weight_Computation(xiwe, ti, scalewe, wwe, Zallwe)
        ...
        secondwe = MAPE(Zallwe, originals, ti)  # 求这个时间戳下的mape
        Mapewe.append(secondwe)

        print("first:", first)
        print("firstwe:", firstwe)
        #供调试用
        print("\n******************************\n对应的用户权重为：",w[ti])
...
def initial(maxepsiono,A,B):
    ...
    sensitivity = 5        #预定义的任务敏感度，所有数据都是天气温度，敏感度一致，设为5
    timestamps = 40    #预定义的时间戳范围
    users = 16     #预定义的用户数
    objects = 26     #预定义的任务数量
    epsiono1 = random.uniform(1,1.5)
    epsiono2 = random.uniform((A/B)*maxepsiono,(A/B)*maxepsiono+0.5)     #这里的范围也需要进一步确定
    ...
    for user in range(users):  # 这是随机选择用户隐私需求，但是对用户较少的情况下，有点不那么随机；先放在这里，留作备用
        ...
        for timestamp in range(1,timestamps):    #初始化的时候epsiono在每个时间戳下，对于同一用户的隐私预算应该是一样的
            scale[timestamp]=scale[0]
            scalewe[timestamp] = scalewe[0]

    #scalewe = deepcopy(scale)
...
if __name__ == '__main__':
    ...
    w = Twodimension_Init(40,16,0.0625)   #20个时间戳5个users，初值为0.01
    wwe = Twodimension_Init(40, 16, 0.0625)  # 20个时间戳5个users，初值为0.01
    scale = Threedimension_Init(40,16,26,0.0001)        #20个时间戳5个user6个objects，初值为0.0001
    scalewe = Threedimension_Init(40, 16, 26, 0.0001)  # 20个时间戳5个user6个objects，初值为0.0001
    TimeAllocate()
