...
#maxepsilon = 0.03    #这个值还不确定，先赋值一个
maxepsilon = 1
w=[]            #表示时间戳中，所有用户对应的weight，是一个二维数组
wwe = []
scale=[]               #epsilon是用户端传上来的，也需要维护一个表吧？每一个用户对应着一个epsilon，同时也有时间标签，
                        #同时对于每一个用户的不同任务也不一样， 所以是一个三维数组
scalewe=[]
epsilonall = []  # 一维数组，记录每一个用户对应的epsilon
...
def TimeAllocate():
    ...
    global alpha, beta, w,scale,lamuda,scalewe,wwe,epsilonall

    #先进行初始化操作——epsilon和weight

    A = PrivacyLevelConfirmation(lamuda, rate=1 - alpha)
    B = PrivacyLevelConfirmation(lamuda, rate=1 - alpha - beta)  # k值越大，A/B的值越小，最小为0.5；
    epsilonall ,epsilonlist=initial(maxepsilon, A, B)           #这里不需要返回值，epsilon和weight都是全局变量


    #从第一个时间戳开始，先对用户加噪
    tilast = 0      #记第一个更新点为tilast
    tilastwe = 0  # 记第一个更新点为tilast
    Mae =[]         #用一个一维数组记录80个时间点内，所有的mae值，然后求平均
    Maewe = []  # 用一个一维数组记录80个时间点内，所有的mae值，然后求平均
    Mape = []  # 用一个以一维组记录80个时间点内，所有的mape值，然后求平均
    Mapewe = []  # 用一个一维数组记录80个时间点内，所有的mape值，然后求平均
    ...
    for ti in range(79):        #先考虑有20个时间戳
        ...
        xi=perturbation(scale,originals,ti)     #xi记录加噪后的数据，是一个二维数组
        scale,tilast,objectaita,adaptive_user=adaptive_sample(tilast,ti,scale,w,epsilonlist,adaptive_user)
        Zall,sumepsilon1 = Weight_Computation(xi, ti, scale, w, Zall)  # w考虑返回
        ...
        Zallwe,sumepsilon2 = Weight_Computation1(xi, ti, scalewe, wwe, Zallwe)      #这里scale不参与计算，只是用来算sumepsilon
        #Zallwe = Weight_Computation1(xiwe, ti, scalewe, wwe, Zallwe)
        #Zallwe = Weight_Computation1(xi, ti, scalewe, wwe,Zallwe)  # w考虑返回

        #epsilonwe, tilastwe = adaptive_sample(tilastwe, ti, epsilonwe, wwe)
    ...
    with open('.//data//result.txt', 'a+', encoding='utf-8') as f:
        f.write(str(sumepsilon2) + '\n' + str(sumepsilon1) + '\n' + str(first1) + '\n' + str(first1we)+ '\n' + str(second1)+ '\n' + str(second1we)+ '\n'+ '\n')

    with open('.//data//adaptive_user.txt', 'a', encoding='utf-8') as f:
        f.write(str(adaptive_user)+'\n')

def initial(maxepsilon,A,B):
    #对epsilon需要全部赋值；第一次的weight值也是在这里赋值
    global alpha,beta,w,wwe,scale,scalewe
    sensitivity = 1        #预定义的任务敏感度，所有数据都是天气温度，敏感度一致，设为5
    timestamps = 80    #预定义的时间戳范围
    users = 50     #预定义的用户数
    objects = 4     #预定义的任务数量
    epsilon1 = random.uniform(0.2,0.25)                    #高隐私级别
    epsilon2 = random.uniform((A/B)*maxepsilon-0.003,(A/B)*maxepsilon-0.0015)     #这里的范围也需要进一步确定
    epsilon3 = maxepsilon                                            #最大的epsilon值

    epsilonlist =[epsilon1,epsilon2,epsilon3]

    #pivot = beta/(alpha+beta)
    timestamp =0            #初始化的时候先只对ti=0时刻进行初始化，epsilon后面的时候均复制
    for user in range(users):             #这是随机选择用户隐私需求，但是对用户较少的情况下，有点不那么随机；先放在这里，留作备用
        ratio = random.random()
        if ratio < alpha :                     #高隐私需求
            for object in range(objects):
                scale[timestamp][user][object] =  sensitivity/epsilon1
            epsilonall.append(epsilon1)
        elif ratio < beta+alpha :                         #中隐私需求
            for object in range(objects):
                scale[timestamp][user][object] = sensitivity/ epsilon2
            epsilonall.append(epsilon2)
        else:                                               #低隐私需求
            for object in range(objects):
                scale[timestamp][user][object] = sensitivity/ epsilon3
            epsilonall.append(epsilon3)             #保存每一用户对应的epsilon的值
    for user in range(users):
        w[0][user] = 1/users            #第一次都是均分的weight
        wwe[0][user] = 1 / users  # 第一次都是均分的weight

        for timestamp in range(1,timestamps):    #初始化的时候epsilon在每个时间戳下，对于同一用户的隐私预算应该是一样的
