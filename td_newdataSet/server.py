...
gamma = 0.0005
#rho=0.4         #是动态budget调整中需要用到的一个参数;3是对应每4个间隔更新；5是对应每5个间隔更新;14对应6个间隔更新,30对应7个间隔
rho=0.02

w=[]            #表示时间戳中，所有用户对应的weight，是一个二维数组
K=100     #用户数量，这里先这样表示，最后直接换成具体实数
#scale=[]               #epsilon是用户端传上来的，也需要维护一个表吧？每一个用户对应着一个epsilon，同时也有时间标签，所以是一个和weight一样的二维数组
newtime = -1        #新的更新时间，
#newuser= []                    # 需要更新的用户id，是一个一维数组，因为需要更新的用户
#maxepsilon = 0.03
#maxepsilon = 0.03
maxepsilon = 1
objectaita = [0.002,maxepsilon]            #可调整的epsilon的阈值
#PI=0.035       #预设的一个epsilon值,电压还受温度影响，这个可能误差会有点大
#PI = 0.04
PI =0.02
newepsilon = 0         #是一个值，对应着每个object的更新的那个值
sensitivity = 0.01
...
def adaptive_sample(tilast , tj,scale,w,epsilonlist,adaptive_user):  # 传入：两个参数上一个更新点的时间和当前时间点
    global  K,rho,newtime,newepsilon,sensitivity,maxepsilon,objectaita
    objectaita[0] = epsilonlist[1] - 0.000001
    newuser = []  # 需要更新的用户id，是一个一维数组，因为需要更新的用户
    newuser[:] = []                 #初始化为空
    ...
    for time1 in range(1,tj - tilast+1):       #从ti开始遍历，到tj-1，time1表示的就是detaT(time是关键字），
        ...
        for userid in range(len(w[0])):         #对于每一个用户而言，都需要做一次判断，是否需要更新epsilon
            # sensitivity2 = np.array(sensitivity)
            # sensitivity2max = sensitivity2/maxepsilon
            # sensitivity2max = sensitivity2max.tolist()      #处理list不能直接除int型变量的问题
            if scale[tilast+time1][userid][0] == sensitivity/maxepsilon :        #不对低隐私用户进行更新;对应低隐私用户而言，他的每一个object都是5/8，这个是不变的，所以可以拿来直接做判断
                continue        #跳过
            else:                       #高、中隐私用户更新判断
                ...
                #     if abs(detaW) > (math.sqrt(epsilon) / K):           #这里换成大写的epsilon是表示这是一个固定值
                #         newtime = time1 + tilast
                #         newuser = userid
                #     elif (time1-1) * (time1 - 2)*(2 * (time1) - 3) * epsilon / 6 > rho:               #间隔时间1、2，都是0.设计的很巧妙，意思说不用考虑这段时间的累积误差了
                #         newtime = time1 + tilast
                #         newuser = userid
                #     else:
                #         newtime = tilast
                ...
                if abs(detaW) > (math.sqrt(PI) / K):  # 这里换成大写的epsilon是表示这是一个固定值
                    newtime = time1 + tilast
        ...
        if newuser :             #说明有用户需要更新
            ...
            #adaptive_user = len(pending_user)
        else:                       #不更新
            newtime = tilast

            # if newtime != tilast :            # 如果需要更新，那我们需要调整新的epsilon,而且是只对需要调整的用户做调整，所以还要维护一个需要调整的用户列表
            #     scale, tilast = Choosen_Time_Users(newtime, newuser, scale, tj)          #对更新时间点newtime上的需要更新的用户newuser的每一个object都更新
                ...
                #     #newepsilon = scale[tilast+time1][newuser][object]          #ti时间戳里面的需要更新的用户对应的epsilon值
                #     newepsilon = scale[tj+1][newuser][object]             #我要更新的是下一个时间点，扰动的数据
                #     #newepsilon = apdate_epxiono(newepsilon)            #产生新的epsilon
                #     #这里需要区分一下，怎么把两个区间分开；并且是对整个用户；可以考虑直接用两个列表，都是除以maxepsilon之后的
                #     if newepsilon > sensitivity1aita[1][object] and newepsilon < sensitivity1aita[0][object]:  #表示这是中隐私              # 所以server还需要保存两个隐私预算的范围
                #         newepsilon = apdate_scale(newepsilon)  # 产生新的epsilon
                #         if newepsilon < sensitivity1aita[1][object]:        #新的sensitivity/epsilon小于低隐私的阈值
                #             newepsilon = sensitivity1aita[1][object]
                #     else:                   #表示这是高隐私用户
                #         newepsilon = apdate_scale(newepsilon)  # 产生新的epsilon
                #         if newepsilon < sensitivity1aita[0][object]:  # 新的sensitivity/epsilon小于中隐私的阈值
                #             newepsilon = sensitivity1aita[0][object]
                #
                #     scale[tj+1][newuser][object]=newepsilon           #我要更新的是下一个时间点，扰动的数据
                # tilast=newtime
                # print("userid:",newuser)
                # return scale,tilast               #当出现更新的时候我们程序就可以返回了——更新后的epsilon列表和最新的更新点
        #print("新的更新点：",tilast)
...
def apdate_scale1(scale):
    newepsilon = random.uniform(0.66,0.90)*scale
    return newepsilon

def apdate_scale(scale):
    global gamma,sensitivity

    newepsilon =  sensitivity/scale +gamma
    newscale = sensitivity / newepsilon
...
def Choosen_Time_Users(newtime,newusers,scale,tj,adaptive_user):              #更新这一时间点上的部分用户的scale
    global sensitivity,objectaita      #引用上面的敏感度和最大的epsilon值
    ...
    for newuser in newusers:                #取列表中的用户id
        # newscale = scale[tilast+time1][newuser][object]          #ti时间戳里面的需要更新的用户对应的epsilon值
        ...
        if newscale > scaleaita[1] and newscale < scaleaita[0]:  # 表示这是中隐私              # 所以server还需要保存两个隐私预算的范围
            ...
            if newscale < scaleaita[1]:  # 新的sensitivity/epsilon小于低隐私的阈值
                newscale = scaleaita[1]
                adaptive_user +=1
        elif newscale > scaleaita[0] :  # 表示这是高隐私用户
            newscale = apdate_scale(newscale)  # 产生新的scale
            if newscale < scaleaita[0]:  # 新的sensitivity/epsilon小于中隐私的阈值
        ...
        print("userid:", newuser)
    tilast = newtime
    return scale, tilast,adaptive_user  # 当出现更新的时候我们程序就可以返回了——更新后的epsilon列表和最新的更新点
...
def Weight_Computation(xi,ti,scale,w,Zall):     #weight估计;需要用户上传的数据xi——表示t=i的时间里面所有用户对所有任务上传的数据（加噪后），是一个二维数组
    ...
    #接着求这个时间点上，所有用户到真值的距离之和，以及所有用户对应的epsilon之和
    ...
    # sumepsilon = 0    #记录所有用户的、所有任务的scale之和
    # for user in range(len(xi)):
    #     sumepsilon = sumW(scale[ti][user]) + sumepsilon

    epsilonall = []             #每次在weight_computation函数中重新计算总的epsilon值
    epsilonall[:] = []      #初始化为空；由于每次的scale[tj][userid][0]代表了这个时间戳下这个用户的scale（单个用户有多个obj，但是scale都一样）
    for user in range(len(xi)):      #二维数组xi，对应直接求的len即为其行数，表示用户数目
        epsilonall.append(sensitivity/scale[ti][user][0])

    #计算所有用户的epsilon值
    sumepsilon = sumW(epsilonall)               #这里还是要用sensitivity/scale[ti][user][0],所以要传入保存的epsionall，
                                        # 但是还没有记录更新的epsilonall。待改进
    print("用户的总epsilon值为：",sumepsilon)
    # with open('.//data//sumepsilon.txt', 'a+', encoding='utf-8') as f:
    #     f.write(str(sumepsilon)+'\n')

#下面求对每个用户调整的weight
    for i in range(len(xi)):             #表示用户id
        w[ti+1][i] = -math.log((sumLti[i]/sumallLti)*math.log(3-(epsilonall[i]/sumepsilon),2),2) #这个是对下一个时间戳的weight调整
        #w[ti+1][i] = -math.log(sumLti[i]/sumallLti,2)      #第二种更新方案
    return Zall , sumepsilon
...
def Weight_Computation1(xi,ti,scale,wwe,Zallwe):     #weight估计;需要用户上传的数据xi——表示t=i的时间里面所有用户对所有任务上传的数据（加噪后），是一个二维数组
    ...
    # sumepsilon = 0    #记录所有用户的、所有任务的epsilon之和
    # for user in range(len(xi)):
    #     sumepsilon = sumW(scale[ti][user]) + sumepsilon
    epsilonall = []             #每次在weight_computation函数中重新计算总的epsilon值
    epsilonall[:] = []      #初始化为空；由于每次的scale[tj][userid][0]代表了这个时间戳下这个用户的scale（单个用户有多个obj，但是scale都一样）
...
#下面求对每个用户调整的weight
    for i in range(len(xi)):             #表示用户id
        #w[ti+1][i] = -math.log((sumLti[i]/sumallLti)*math.log(3-(sumW(scale[ti][i])/sumepsilon),2),2) #这个是对下一个时间戳的weight调整
        wwe[ti+1][i] = -math.log(sumLti[i]/sumallLti,2)      #第二种更新方案
    return Zallwe,sumepsilon

def change_all(tilast,tj,scale,w,epsilonlist,adaptive_user):  # 传入：两个参数上一个更新点的时间和当前时间点
    global  K,rho,newtime,newepsilon,sensitivity,maxepsilon,objectaita
    objectaita[0] = epsilonlist[1] - 0.000001
    newuser = []  # 需要更新的用户id，是一个一维数组，因为需要更新的用户
    newuser[:] = []                 #初始化为空
